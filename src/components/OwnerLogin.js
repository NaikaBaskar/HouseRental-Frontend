    ...
    .then(res =>res.json())
      .then (res => {
        ...
        if (res.msg === "approved") {
          ...
          fetch('https://house-rental-backend.herokuapp.com/owner/changePassword', {
          method: 'PUT',
          headers: {
              'Content-Type': 'application/json',
            },
...
goback = () =>{
  ...
  render () {
    ...
    return (
      ...
      <h1 className="m-3" style={{marginTop:"20px",marginBottom:"25px"}}>{this.state.forgot ? "Update Password" : "Owner Login"}</h1>
        <div>
